...
export default function LocationsManager({ locations, onAdd, onDelete, onClose }) {
  ...
  return (
    ...
    >
      <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full sm:max-w-lg max-h-[85vh] overflow-y-auto shadow-modal animate-slide-up">
        ...
        <div className="px-5 pb-6 pt-2">
          ...
          {locations.length > 0 ? (
            <div className="grid grid-cols-2 gap-3 mb-5">
              {locations.map(loc => (
                <div key={loc.id} className="rounded-2xl border-2 border-gray-100 overflow-hidden">
                  {loc.photo ? (
                    <img src={loc.photo} alt="" className="w-full h-28 object-cover" />
                  ) : (
                    <div className="w-full h-28 bg-lavender-50 flex items-center justify-center text-4xl">📍</div>
                  )}
                  <div className="flex items-center justify-between px-2.5 py-2 bg-white">
                    <span className="font-sans font-bold text-sm text-gray-700 truncate flex-1">{loc.name}</span>
                    <button
                      onClick={() => onDelete(loc.id)}
                      className="flex-shrink-0 w-6 h-6 rounded-full bg-red-50 text-red-300 hover:bg-red-100 hover:text-red-400 transition-all flex items-center justify-center text-xs ml-1"
                    >✕</button>
                  </div>
                </div>
              ))}
            ...
            <p className="font-sans text-sm text-gray-400 text-center mb-5 py-4">
              No locations yet. Add your first one below!
            </p>
          )}

          <div className="bg-lavender-50/50 rounded-2xl p-4 border-2 border-dashed border-lavender-200">
            <h3 className="font-sans font-bold text-sm text-gray-600 mb-3">Add new location</h3>
            <form onSubmit={handleAdd} className="space-y-3">
              <input
                ...
                className="w-full rounded-2xl border-2 border-lavender-200 px-4 py-2.5 font-sans text-sm focus:outline-none focus:border-blush-300 bg-white transition-colors"
              />
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 rounded-xl font-sans font-bold text-sm text-blush-400 bg-white border-2 border-blush-200 hover:bg-blush-50 transition-all"
                >
